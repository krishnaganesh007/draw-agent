...
def draw_rectangle(
    ...
    try:
        ...
        print("DEBUG: Clicked Draw button.")
        time.sleep(0.2)
        print("DEBUG: Starting to draw rectangle.")
        ...
        print("DEBUG: Finished drawing rectangle.")
...
def add_text_in_rectangle(
    ...
    Returns:
      dict with status/message and a content list that includes a TextContent block.
    """
    try:
        if is_autodraw_open():
            print("DEBUG: AutoDraw tab is open, switching to it.")
        ...
        # Writing text: click text tool, then click on canvas and type with interval
        time.sleep(1.0)
        pyautogui.click(45, 450, duration=0.6)
        print("DEBUG: Clicked Text tool.")
        time.sleep(0.4)
        pyautogui.moveTo(300, 600)
        pyautogui.click()
        time.sleep(0.25)
        print("DEBUG: Clicked on canvas to start typing.")
        pyautogui.write(text, interval=0.8)  # Use interval to avoid missing chars

        msg = f'Typed "{text}" at (400, 600) on AutoDraw.'
        return {
            "status": "success",
            "message": msg,
